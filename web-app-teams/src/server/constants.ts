...
export const SELF_EXTERNAL_API = {
    URL: process.env.SELF_EXTERNAL_API_URL?.trim() || "",
...
export const TEAMS_TAB = {
    SEARCH: { ID: process.env.TEAMS_TAB_ID_SEARCH?.trim() || "" },
    MYPROFILE: { ID: process.env.TEAMS_TAB_ID_MYPROFILE?.trim() || "" },
    FAVOURITES: { ID: process.env.TEAMS_TAB_ID_FAVOURITES?.trim() || "" },
    PARTNERCONNECTIONS: { ID: process.env.TEAMS_TAB_ID_PARTNERCONNECTIONS?.trim() || "" },
    APIKEYS: { ID: process.env.TEAMS_TAB_ID_APIKEYS?.trim() || "" }
}

export const CLIENT_CONFIG = {
    MANUAL_MODE: JSON.parse((process.env.MANUAL_MODE || "False").toLocaleLowerCase().trim())
}